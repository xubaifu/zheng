...
<script>
//创建子集表单(动态获取列并拼接成表单)
function createSubsetClumns(tableId, tableName, organizationId, rows){
		...
		success: function(data) {
	        ...
	        if(data.rows != null && data.rows.length != 0){
	        	...
	        	if(rows == null || typeof rows == "undefined"){
	        		...
	        		$("#subsetCreate").append(
		        			'<div class="form-group text-right dialog-buttons">'+
		        				'<a class="waves-effect waves-button" href="javascript:;" onclick="updateSubsetDataSubmit(\''+tableId+'\',\''+tableName+'\',\''+organizationId+'\')">保存</a>'+
		        				'<a class="waves-effect waves-button" href="javascript:;" onclick="subsetDialog.close();">取消</a>'+
		        			'</div>'
		        		)
	        	}
...
//修改数据
function updateSubsetDataSubmit(tableId, tableName, organizationId){
	...
	$.ajax({
        type: 'post',
        url: '${basePath}/manage/organization/updateSubsetData/'+tableName+'/'+subId,
        data: {"data":JSON.stringify(param)},
        dataType: "json",
       /*  beforeSend: function() {
        }, */
